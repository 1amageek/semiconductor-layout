...
/// Mutable uniform-grid index over flattened shape occurrences.
///
/// This is the `FlatShapeKey` counterpart of ``MutableShapeGridIndex`` for
/// DRC internals, where child-cell occurrences can share the same source UUID
/// but must remain distinct after flattening.
struct MutableFlatShapeGridIndex {
    ...
    private var cells: [CellKey: Set<FlatShapeKey>] = [:]
    private var coveredCellsByKey: [FlatShapeKey: [CellKey]] = [:]

    init(
        boundingBoxes: [(key: FlatShapeKey, box: LayoutRect)],
        cellSize: Double
    ) {
    ...
    mutating func insert(key: FlatShapeKey, box: LayoutRect) {
        remove(key: key)
    ...
    mutating func remove(key: FlatShapeKey) {
    ...
    func neighbours(of box: LayoutRect, margin: Double = 0) -> [FlatShapeKey] {
        candidateKeys(of: box, margin: margin).sorted()
    }

    func candidateKeys(of box: LayoutRect, margin: Double = 0) -> Set<FlatShapeKey> {
        let probe = box.expanded(by: margin, margin)
        let keys = cellKeys(overlapping: probe)
        guard !keys.isEmpty else { return [] }
        var found: Set<FlatShapeKey> = []
        for key in keys {
